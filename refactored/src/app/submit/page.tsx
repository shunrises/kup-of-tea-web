...
function SortableMemberItem({
  ...
  return (
    ...
    >
      ...
      <div
        className={`grid grid-cols-1 md:grid-cols-2 gap-4 ${!isSoloArtist ? 'pl-6' : ''}`}
      >
        <div className="space-y-2">
          <Label htmlFor={`memberName-${member.id}`}>
            {isSoloArtist ? '활동명' : '멤버 이름'}
          </Label>
          <Input
            ...
            placeholder={
              isSoloArtist ? '활동명을 입력하세요' : '멤버 이름을 입력하세요'
            }
            required
          />
          {errors?.name && (
...
export default function MusicGroupForm() {
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        setErrors((prev) => ({
          ...prev,
          logoImage: '이미지 크기는 5MB 이하여야 합니다.',
        }))
        return
      }
  ...
  const handleTickerChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '') // 소문자, 숫자, 하이픈만 허용
    setTicker(value)
    ...
    if (value.trim() === '') {
      setErrors((prev) => ({
        ...prev,
        ticker: 'Ticker를 입력해주세요. (영문, 숫자, 하이픈만 가능)',
      }))
    } else if (value.length < 2) {
      setErrors((prev) => ({
        ...prev,
        ticker: 'Ticker는 최소 2자 이상이어야 합니다.',
      }))
    } else {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        setMembers(
          members.map((member) =>
            member.id === memberId
              ? { ...member, profileImage: e.target?.result as string }
              : member,
          ),
        )
  ...
  const addMember = () => {
    ...
    if (groupType && groupType !== 'female_solo' && groupType !== 'male_solo') {
      setMembers([
        ...members,
        { id: Date.now().toString(), name: '', profileImage: null },
      ])
    }
  }
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      ...
      case 2: // 기본 정보
        ...
        if (!ticker.trim()) {
          newErrors.ticker = 'Ticker를 입력해주세요.'
          isValid = false
        } else if (ticker.trim().length < 2) {
          newErrors.ticker = 'Ticker는 최소 2자 이상이어야 합니다.'
          isValid = false
        } else if (!/^[a-z0-9-]+$/.test(ticker.trim())) {
          newErrors.ticker =
            'Ticker는 영문 소문자, 숫자, 하이픈만 사용 가능합니다.'
          isValid = false
        }
        break
      case 3: // 멤버 정보
        const memberErrors: {
          [key: string]: { name?: string; profileImage?: string }
        } = {}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!normalizedTicker) {
        throw new Error('Ticker를 입력해주세요.')
      }

      if (normalizedTicker.length < 2) {
        throw new Error('Ticker는 최소 2자 이상이어야 합니다.')
      }

      if (!/^[a-z0-9-]+$/.test(normalizedTicker)) {
        throw new Error('Ticker는 영문 소문자, 숫자, 하이픈만 사용 가능합니다.')
      }

      // refactored 프로젝트의 submit-team API로 데이터 전송
      const response = await fetch('/api/submit-team', {
        ...
        body: JSON.stringify({
          ...
          logo: logoImage, // base64 또는 R2 URL
          groupPhoto: '', // 그룹 사진이 있으면 추가
          groupType: groupType, // 'boy_group', 'girl_group', 'coed_group', 'male_solo', 'female_solo'
          members: members.map((member) => ({
            name: member.name,
            profileImage: member.profileImage, // base64 또는 R2 URL
          })),
          submittedBy: null, // 제출자 정보 (선택사항)
        }),
      })
      ...
      toast.success(
        '팀 정보가 승인 대기 중입니다. 승인 후 서비스에 표시됩니다.',
      )
            ...
            <div className="space-y-4">
              ...
              <RadioGroup
                ...
                onValueChange={(value: string) =>
                  handleGroupTypeChange(value as GroupType)
                }
                  ...
                  <div className="flex items-center space-x-3 border rounded-lg p-4 cursor-pointer hover:bg-muted/50 transition-colors">
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">
                        {groupTypeDescriptions.female_solo}
                      </p>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-3 border rounded-lg p-4 cursor-pointer hover:bg-muted/50 transition-colors">
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">
                        {groupTypeDescriptions.male_solo}
                      </p>
                    </Label>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="flex items-center space-x-3 border rounded-lg p-4 cursor-pointer hover:bg-muted/50 transition-colors">
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">
                        {groupTypeDescriptions.girl_group}
                      </p>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-3 border rounded-lg p-4 cursor-pointer hover:bg-muted/50 transition-colors">
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">
                        {groupTypeDescriptions.boy_group}
                      </p>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-3 border rounded-lg p-4 cursor-pointer hover:bg-muted/50 transition-colors">
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">
                        {groupTypeDescriptions.coed_group}
                      </p>
                    </Label>
                  </div>
                </div>
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="영문 그룹명 또는 약자를 입력하세요"
                required
                pattern="[a-z0-9-]+"
                className="lowercase"
              />
              ...
              <p className="text-xs text-muted-foreground">
                영문 소문자, 숫자, 하이픈(-)만 사용 가능합니다. URL에 사용되는
                고유 식별자입니다.
              </p>
            </div>
          </>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-mono text-sm">{ticker || '없음'}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">
                      그룹 타입
